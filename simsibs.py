...
def freq_analysis_randomvariant(results, sample_indiv, num_iters):
            ...
            for lemma, variants in individual.inputvariants.items():
                ...
                if i == sample_indiv: #only the last developing
                    ...
                    if lemma in community.init_variants:
                        variants_by_varlemma[lemma].append(get_categoricalvariant(variants))
                    else:
                        variants_by_reglemma[lemma].append(get_categoricalvariant(variants))
    ...
    for lemma, tokenfreq in sorted(tokenfreqs_by_lemma.items(),  key=lambda x: x[1], reverse=True):
        ...
        if lemma in community.init_variants:
            init_variant = "TRUE"
        analyzedlemmas.append((community.interactionfunc.__name__,len(community.init_variants), community.lemmas.N, init_variant, lemma, rank, tokenfreq/num_iters, variants_by_lemma[lemma].count(1)/denom, variants_by_varlemma[lemma].count(1)/vardenom, variants_by_reglemma[lemma].count(1)/regdenom))
...
def run_experiment_multiproc(num_lemmas, comm_size, num_children, ninit, ncontinue, relevants, irregnum, num_trials, num_iters, gen_interactionfunc):
    ...
    while completed < num_trials:
        ...
        for i in range(0,maxp):
            print(completed, end=" ")
            completed += 1
            p = Process(target=worker, args = (completed, spent_communities, lemmas, num_lemmas, comm_size, num_children, ninit, ncontinue, relevants, irregnum, num_trials, num_iters, gen_interactionfunc))
            ps.append(p)
            p.start()
        for p in ps:
            p.join()
